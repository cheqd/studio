...
import { CredentialPayload, PresentationPayload, W3CVerifiableCredential, W3CVerifiablePresentation } from '../types/vc-data-model'
...
} from '@eengineer1/did-jwt-vc'

import { decodeJWT } from '@eengineer1/did-jwt'
...
export class CredentialIssuer implements IAgentPlugin {
  ...
  async verifyCredential(args: IVerifyCredentialArgs, context: IContext): Promise<boolean> {
    ...
    if (typeof credential === 'string' || (<VerifiableCredential>credential)?.proof?.jwt) {
      ...
      try {
        const verification = await verifyCredentialJWT(jwt, resolver, { proofPurpose: 'verificationMethod' })
        return true
      } catch (e: any) {
