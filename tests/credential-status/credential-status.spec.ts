...
const PAYLOADS_BASE_PATH = './tests/payloads/credential-status';
...
test('Create encrypted StatusList2021 for user with testnet role but network is mainnet', async ({ request }) => {
	const response = await request.post(`/credential-status/create/encrypted`, {
		data: JSON.parse(
			fs.readFileSync(
				`${PAYLOADS_BASE_PATH}/credential-status-create-encrypted-without-permissions.json`,
				'utf-8'
			)
		),
		headers: {
...
test('Create unencrypted StatusList2021 for user with testnet role but network is mainnet', async ({ request }) => {
	const response = await request.post(`/credential-status/create/unencrypted`, {
		data: JSON.parse(
			fs.readFileSync(
				`${PAYLOADS_BASE_PATH}/credential-status-create-unencrypted-without-permissions.json`,
				'utf-8'
			)
		),
		headers: {
...
test('Update encrypted StatusList2021 for user with testnet role but network is mainnet', async ({ request }) => {
	const response = await request.post(`/credential-status/update/encrypted`, {
		data: JSON.parse(
			fs.readFileSync(
				`${PAYLOADS_BASE_PATH}/credential-status-update-encrypted-without-permissions.json`,
				'utf-8'
			)
		),
		headers: {
...
test('Update Unencrypted StatusList2021 for user with testnet role but network is mainnet', async ({ request }) => {
	const response = await request.post(`/credential-status/update/unencrypted`, {
		data: JSON.parse(
			fs.readFileSync(
				`${PAYLOADS_BASE_PATH}/credential-status-update-unencrypted-without-permissions.json`,
				'utf-8'
			)
		),
		headers: {
