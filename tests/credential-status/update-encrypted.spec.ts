...
test('[Negative] It cannot update an encrypted statusList2021 in mainnet network for user with testnet role', async ({ request }) => {
	const response = await request.post(`/credential-status/update/encrypted`, {
		data: JSON.parse(fs.readFileSync(`${PAYLOADS_PATH.CREDENTIAL_STATUS}/update-encrypted-without-permissions.json`, 'utf-8')),
