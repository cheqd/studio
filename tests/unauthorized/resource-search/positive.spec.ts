...
import { TESTNET_DID, TESTNET_RESOURCE_ID } from '../constants';

test('/resource/search/{did} with resourceId query parameter', async ({ request }) => {
    const response = await request.get(`/did/search/${TESTNET_DID}?resourceId=${TESTNET_RESOURCE_ID}`);
    expect(response.ok()).toBeTruthy();
    
    const body = await response.json();
    const expected = JSON.parse(fs.readFileSync('./tests/unauthorized/payloads/resource-search/resource.json', 'utf-8'));
