...
test('[Negative] It cannot reinstate credential in mainnet network for user with testnet role', async ({ request }) => {
	const response = await request.post(`/credential/reinstate`, {
		data: JSON.parse(
			fs.readFileSync(`${PAYLOADS_PATH.CREDENTIAL}/reinstate-credential-without-permissions.json`, 'utf-8')
		),
