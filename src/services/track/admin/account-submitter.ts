    ...
    async submitStripeAccountCreate(operation: ISubmitOperation): Promise<void> {
        ...
        try {
            ...
            if (account.lastResponse.statusCode !== StatusCodes.OK) {
                await this.notify({
                    message: EventTracker.compileBasicNotification(`Failed to create Stripe account with name: ${data.name}.`, operation.operation),
                    severity: 'error',
                } as INotifyMessage);
                return;
            }
            ...
            await this.notify({
                message: EventTracker.compileBasicNotification(`Stripe account created with name: ${data.name}.`, operation.operation),
                severity: 'info',
            } as INotifyMessage);
        } catch (error) {
            await this.notify({
                message: EventTracker.compileBasicNotification(`Failed to create Stripe account with name: ${data.name as string}.`, operation.operation),
                severity: 'error',
            } as INotifyMessage);
        }
    }
