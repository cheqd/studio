    ...
    return (target: any, propertyKey: string, descriptor: PropertyDescriptor) =>  {
        ...
        descriptor.value = async (...args: any) => {
            try {
                ...
                eventTracker.notify({
                    message: EventTracker.compileBasicNotification(`Error while calling function: ${propertyKey}: ${(error as Record<string, unknown>)?.message || error}`),
                    severity: 'error',
...
export class SubscriptionSubmitter implements IObserver {
    ...
    async submitSubscriptionCreate(operation: ISubmitOperation): Promise<void> {
        ...
        try{
            ...
            if (customers.length !== 1) {
                this.notify({
                    message:  EventTracker.compileBasicNotification(`It should be only 1 Stripe account associated with CaaS customer. Stripe accountId: ${data.stripeCustomerId}.`, operation.operation),
                    severity: 'error',
                })
            }
            ...
            this.notify({
                message: EventTracker.compileBasicNotification(`Failed to create a new subscription with id: ${data.subscriptionId} because of error: ${(error as Error)?.message || error}`, operation.operation),
                severity: 'error',
            })
        }
    }
    ...
    async submitSubscriptionUpdate(operation: ISubmitOperation): Promise<void> {
        ...
        try {
            ...
            this.notify({
                message: EventTracker.compileBasicNotification(`Failed to update subscription with id: ${data.subscriptionId} because of error: ${(error as Error)?.message || error}`, operation.operation),
                severity: 'error',
            })
        }
    }
    ...
    async submitSubscriptionCancel(operation: ISubmitOperation): Promise<void> {
        ...
        try {
            ...
            this.notify({
                message: EventTracker.compileBasicNotification(`Failed to cancel subscription with id: ${data.subscriptionId} because of error: ${(error as Error)?.message || error}`, operation.operation),
                severity: 'error',
            })
        }
    }
