    ...
    public async stripeSync(customer?: CustomerEntity, user?: UserEntity): Promise<void> {
        ...
        for (const subscription of subscriptions.data.sort((a, b) => a.created - b.created)){
            ...
            if (!existing) {
                ...
                if (!res) {
                    eventTracker.notify({
                        message: EventTracker.compileBasicNotification(`Cannot create a new subscription with id ${subscription.id}`, 'Subscription syncronization'),
                        severity: 'error',
                    });
                }
                eventTracker.notify({
                    message: EventTracker.compileBasicNotification(`New subscription with id ${subscription.id} created`, 'Subscription syncronization'),
                    severity: 'info',
                });
            } else {
                ...
                if (!res) {
                    eventTracker.notify({
                        message: EventTracker.compileBasicNotification(`Cannot update subscription with id ${subscription.id}`, 'Subscription syncronization'),
                        severity: 'error',
                    });
                }
                eventTracker.notify({
                    message: EventTracker.compileBasicNotification(`Subscription with id ${subscription.id} updated`, 'Subscription syncronization'),
                    severity: 'info',
                });
            }
        }
