...
export class CheqdW3CVerifiablePresentationValidator implements IValidator {
	...
	constructor(
		...
		credentialValidators?: IValidator[],
        holderValidators?: IValidator[],
	) {
		if (!proofValidators) {
	...
	validate(presentation: Validatable): IValidationResult {
        ...
        if (credentialResults.some((r) => r.some((cr) => !cr.valid))) {
            return {
                valid: false,
                error: `Presentation verifiableCredential has validation errors: ${credentialResults.map((r) => r.map((cr) => cr.error).join(', ')).join(', ')}`,
            };
        }
